...
TemporizedSlider.previous = function () {
  var pointer = (pointer - 1 >= 0) ? (pointer - 1) : end;
...
TemporizedSlider.next = function () {
  var pointer = (pointer + 1 <= end) ? (pointer + 1) : 0;
...
TemporizedSlider.scheduleNextChange = function () {
  clearTimeout(timeOut);
  var timeOut = setTimeout('TemporizedSlider.play(true)', collection[pointer].time * 1000);
};

TemporizedSlider.defineClicks = function () {
  var play_control = document.getElementById('play_control'),
      pause_control = document.getElementById('pause_control'),
      previous_control = document.getElementById('previous_control'),
      next_control = document.getElementById('next_control');

  play_control.onclick = function () {
    args.controls.fns.play();
  };

  pause_control.onclick = function () {
    args.controls.fns.pause();
  };

  previous_control.onclick = function () {
    args.controls.fns.previous();
  };

  next_control.onclick = function () {
    args.controls.fns.next();
  };
};